function [B,A,A12,A13,A22,A23,C1,WB1,WB2] = train_OHPKL0(X, Y, L, param, nbits,unlabel)
alpha = param.alpha;
beta = param.beta;
lambda = param.lambda;
theta = param.theta;
...
for iter = 1:param.iter
   U = (2 * eye(c)  + theta * W1 * WB1 * W1' + theta * W2 * WB2 * W2')...
...
for iter = 1:param.iter 
    C1 = V*L_';
    B = sign(nbits*alpha*C1*L_ + beta*V );
    
    K = beta*B + alpha*nbits*B*L_'*L_;
    K = K';
